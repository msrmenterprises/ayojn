		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 id="bid_id" style="text-align: center;margin-bottom: 18px !important;">
					Please provide reason to suspand user
				</h4>
			</div>
				...
				<form class="addBidForm" id="addBidForm" method="post" onsubmit="return false"
					...
					<div id="add-opporutnity-form">
						...
						<div class="row">
							...
							<div class="col-md-4">
								<input type="hidden" value="<?php echo e($id); ?>" id="userId"
									   name="userId">
							</div>
						</div>
						<div class="row">
...
<script>
  function submitForm() {
    ...
    if (response != '' && userId != '') {
      $.ajax({
        url: "<?php echo e(url('/')); ?>" + '/controlpanel/user-suspand-comment',
        type: "post",
        async: false,
        data: {userId: userId, suspend_reason: response},
        headers: {"X-CSRF-TOKEN": "<?php echo e(csrf_token()); ?>"},
        success: function (data) {
          data = jQuery.parseJSON(data);
          if (data.status) {
            swal("Updated", data.msg, "success");
            $("#add-reason").modal('hide');
            $("#append-form").html('');
            setTimeout(function () {
              location.reload()
            }, 1500)
          } else {
            swal('Updated', data.msg, 'error');
            $("#add-reason").modal('hide');
            $("#append-form").html('');
          }
        },
