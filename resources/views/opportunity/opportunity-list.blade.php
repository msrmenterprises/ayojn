...
@section('content')
	...
	<section id="feed" class="speakers">
		<div class="container" style="padding-top: 15px;">
			...
			<div class="row"><a href="{{ url('build-opportunity') }}" class="btn btn-primary float-left"
				...
				<table id="table_id" class="display">
					...
					@if(!empty($bids->first()))
						@foreach($bids as $bid)
							<tr>
								<td>#{{ $bid->id }}</td>
								<td><a href="{{ url('vouches/')."/".$bid->id  }}"><span
											class="badge badge-warning">{{ $bid->vouchesResponse->count() }}</span></a>
								</td>
								<td>{{  $bid->hashtag }}</td>
								<td>{{  (!empty($bid->country_name)) ? $bid->country_name->country_name : "-" }}</td>
								<td>{{  (!empty($bid->opportunity_city)) ? $bid->city_name->name : "-" }}</td>
								...
								<td><span id="opportunity_{{ $bid->id }}" class="share-course-filed"
										  style="display: none"> {{ url('share-opportunity')."/" . $bid->share_id }}</span> <a
										href="javascript:void(0)" class="btn btn-primary read-more-btn  share-opp-btn"
										onclick="copyToClipboard('#opportunity_{{ $bid->id }}',{{$bid->status}})">Copy Web link</a></td>
							</tr>
						@endforeach
					@endif
					</tbody>
