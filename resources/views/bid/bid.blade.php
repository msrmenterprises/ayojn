    ...
    <section id="feed" class="speakers">
        <div class="container">
            <div class="row">
                ...
                @endif
                @if($unpaid > 0)
                    <a href="{{ url('unpaid-bid') }}" class="btn btn-primary float-left"
                    ...
                    @if(!empty($bids->first()))
                        @foreach($bids as $bid)
                            ...
                            <!--<td>{{ $bid->contacted_by }}</td> -->
                                ...
                                <td>
                                    <input type="checkbox" onchange='changebid({{ $bid->id }},"{{ $bid->status }}")'
                                           name="toggle-event-bid" data-toggle="toggle" data-on="On"
                                           data-off="Off" {{ ($bid->status == 'On')?'checked':'' }}>
                                </td>
                                <td><a href="{{ url('bid-response/') ."/".base64_encode($bid->id) }}"> <span
                                            class="badge badge-light">{{ $bid->bidResponseWithSpam->count() }}</span> </a></td>
                                <td><?php
                            ...
                            <!--<td><a href="javascript:void(0)" onclick="editBid({{ $bid->id }})"
                                    ...
                                    <select class="form-control" name="specify_bid" id="specify_bid" multiple>
                                        @if(count($sponsorrlist) > 0)
                                            @foreach($sponsorrlist as $sponsor_list)
                                                <option
                                                    value="{{ $sponsor_list->id }}">{{ $sponsor_list->specify_name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
